    ...
    <Modal onClose={onClose} size="2xl" scrollable ariaLabel={subscription ? 'Abonnement bewerken' : 'Nieuw abonnement'}>
      ...
      <form id="sub-form" onSubmit={handleSubmit} className="px-6 py-5 space-y-7">
        ...
        <Section label="Kosten & facturatie">
          ...
          {/* — Bedragen — (zonder periode; periode hoort bij datums want bepaalt de cyclus) */}
          ...
          {(() => {
            ...
            if (isLicSeats) {
              return <FieldGrid cols={3}>{costField}{seatsField}{baseField}</FieldGrid>;
            }
            if (isVariable) {
            ...
            <div className="max-w-xs">
              <Field
                ...
                hint={!fieldErrors.renewal_date
                  ? 'Bepaalt de cycluslengte voor de maandberekening.'
                  : undefined}
              >
                <input
                  type="date"
